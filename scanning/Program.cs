...
namespace scanning
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (string line in AllLines)
            {
                if (lineCounter >= 2)
                {
                    ...
                    if (lineCounter % 2 == 0)
                    {
                        ...
                        var lineSpl = line.Split(' ').Select(l => int.Parse(l)).ToList(); ;
            ...
            Librarys = Librarys.OrderBy(s => s.SignUp).ThenByDescending(s => s.BookaPerDay).ThenByDescending(s => s.NumberOfBooks).ToList();
            ...
            save(content ,Convert.ToString(LibCounter));
            
            void save(string Data,string libCounter)
            {
                filename = @"C:\Users\HP\source\repos\scanning\scanning\bin\Debug" + DateTime.Now.TimeOfDay.Hours
                                                         ...
                                                         + ".txt";
                string Coll = libCounter+Data;
                File.WriteAllText(filename, Coll);
            }
